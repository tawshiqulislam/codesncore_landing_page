...
if (!function_exists('getParam')) {
    ...
    function getParam()
    {
        $parsedUrl = parse_url(url()->current() . '/cnc');
    ...
    // {
    //     $parsedUrl = parse_url(url()->current());
    //     if(!str_contains(url()->current(), 'cnc') && isset($parsedUrl['path'])){
    //         $parsedUrl['path'] = '/cnc' . $parsedUrl['path'];
    //         // dd($parsedUrl, 'if');
    //     }
    ...
    //     }
    //     // dd($parsedUrl);
    //     // if(!str_contains($parsedUrl['host'], 'cnc')){
    //     //     $parsedUrl['host'] = $parsedUrl['host'];
    //     // }
    //     $host = str_replace("www.", "", $parsedUrl['host']);
    //     // dd($host, $parsedUrl, url()->current());
    //     // if it is path based URL, then return {username}
    //     if (strpos($host, env('WEBSITE_HOST')) !== false && $host == env('WEBSITE_HOST')) {
    //         $path = explode('/', $parsedUrl['path']);
    //         // dd(isset($path[2]));
