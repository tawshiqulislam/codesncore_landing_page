...
class BasicController extends Controller
{
    ...
    public function homePageUpdateVideo(Request $request)
    {
        ...
        $data = HomePageText::where('language_id', $lang->id)
        ...
        if (empty($data->video_section_image) && !$request->hasFile('video_section_image')) {
            $request->validate([
                ...
                'video_section_image.required' => 'Background image is required',
                'video_section_image.mimes' => 'Image must be jpeg, jpg, or png',
                'video_section_image.max' => 'Image size should be maximum 1MB'
            ]);
        }

        // Handle background image
        if ($request->hasFile('video_section_image')) {
            $request['video_section_image'] = Uploader::update_picture(
                'assets/front/img/user/home_settings/', 
                $request->file('video_section_image'), 
                $data->video_section_image
            );
        } else {
            $request['video_section_image'] = $data->video_section_image;
        }

        // Handle video URL or slides
        $slides = [];
        
        // Check if we're dealing with a video URL or images
        $isVideoUrl = $request->filled('video_section_url') && 
                    (str_contains($request->video_section_url, 'youtube.com') || 
                    str_contains($request->video_section_url, 'youtu.be'));
        ...
        if ($isVideoUrl) {
            ...
            // Delete existing slide images if switching from slides to video
            if (!empty($data->video_section_url) && $this->isImageString($data->video_section_url)) {
                foreach (explode(',', $data->video_section_url) as $oldImage) {
                    @unlink('assets/front/img/user/video_slides/' . $oldImage);
                }
            }
            
            $request['video_section_url'] = $videoLink;
        } 
        // If slides are uploaded or we have existing images
        elseif ($request->hasFile('video_section_slides') || 
            ($request->filled('video_section_url') && $this->isImageString($request->video_section_url))) {
            
            // First, keep existing slides if they exist and no images were removed
            if (!empty($data->video_section_url) && $this->isImageString($data->video_section_url)) {
                // Handle removed slides if any
                if ($request->filled('removed_slides')) {
                    $removedSlides = json_decode($request->removed_slides, true);
                    $existingSlides = explode(',', $data->video_section_url);
                    $slides = array_diff($existingSlides, $removedSlides);
                    
                    // Delete removed slide images
                    foreach ($removedSlides as $slide) {
                        @unlink('assets/front/img/user/video_slides/' . $slide);
                    }
                } else {
                    $slides = explode(',', $data->video_section_url);
                }
            }
            
            // Add new slides if uploaded
            if ($request->hasFile('video_section_slides')) {
                foreach ($request->file('video_section_slides') as $image) {
                    $filename = uniqid() . '.' . $image->getClientOriginalExtension();
                    $image->move('assets/front/img/user/video_slides/', $filename);
                    $slides[] = $filename;
                }
            }
            
            // If we have slides (either existing or new), update the URL field
            if (!empty($slides)) {
                $request['video_section_url'] = implode(',', $slides);
            }
        }

        $data->update($request->except(['video_section_text']) + [
    ...
    private function isImageString($str)
    {
