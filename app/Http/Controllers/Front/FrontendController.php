...
class FrontendController extends Controller
{
    ...
    //     return $this->userDetailView('cnc');
    ...
    public function index()
    {
        ...
        $data['q_products'] = $user->quote_products()->where([
                ['lang_id', $userCurrentLang->id],
                ['featured', 1]
            ])
                ->orderBy('serial_number', 'ASC')
    ...
    public function userQProducts($domain)
    {
        $user = getUser();
        $id = $user->id;
        ...
        $data['products'] = User\UserProductsQuote::query()
            ->where('user_id', $id)
            ->where('lang_id', $userCurrentLang->id)
            ->orderBy('serial_number', 'ASC')
            ->get();
        $data['distributors'] = $user->brands()
            ->where('distributor', 1)
            ->get() ?? collect([]);
        return view('user-front.quote_product.index', $data);
    }

    public function userQProductsDetail($domain, $slug, $id)
    {
        $user = getUser();
        $data['related_products'] = collect(User\UserProductsQuote::query()->where('user_id', $user->id)->get());
        $data['product'] = User\UserProductsQuote::query()->findOrFail($id);
        // dd($data['related_products']);
        return view('user-front.quote_product.show', $data);
    }

    public function userQProduct_req(Request $request)
    {
    ...
    public function view_gallery(Request $request, $domain)
    {
        $user = getUser();
        $id = $user->id;
        $data['user'] = $user;
        ...
        $images = UserGallery::where('user_id', $id)->where('is_pdf', 0)->get();

        return view('front.gallery.view', compact('images', 'data'));
    }

    public function view_downloads(Request $request, $domain)
    {
        $user = getUser();
        $id = $user->id;
        $data['user'] = $user;
        ...
        $pdf = UserGallery::where('user_id', $id)->where('is_pdf', 1)->get();
        
        return view('front.gallery.view_pdf', compact('pdf', 'data'));
    }
