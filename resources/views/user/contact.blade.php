        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <div class="card-title">{{ __('Update Main Contact') }}</div>
                        </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Add Secondary locations</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('user.contact.update', ['language' => request()->input('language')]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="text" name="contact_form_image" value="1" hidden>
                        <div class="form-group">
                            <label for="">{{ __('Form Title') }}</label>
                            <input type="text" class="form-control" name="contact_form_title" placeholder="Enter title">
                        </div>

                        <div class="form-group">
                            <label for="">{{ __('Form Subtitle') }}</label>
                            <input type="text" class="form-control" name="contact_form_subtitle" placeholder="Enter Subtitle">
                        </div>
                        <div class="form-group">
                            <label>{{__('Address')}}</label>
                            <textarea class="form-control" name="contact_addresses" rows="3"></textarea>
                            <p class="mb-0 text-warning">{{__('Use newline to seperate multiple addresses.')}}</p>
                        </div>
                        ...
                        <div class="form-group">
                            <label>{{__('Map Zoom')}}</label>
                            <input class="form-control" name="map_zoom" value="12">
                        </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Secondary locations</h4>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped">
                            ...
                            <tbody>
                            @foreach($second as $key => $contact)
                                <tr>
                                    <td>{{$contact->contact_form_title}}</td>
                                    <td>{{$contact->contact_form_subtitle}}</td>
                                    <td>{{$contact->contact_addresses}}</td>
                                    <td>{{$contact->contact_numbers}}</td>
                                    <td>{{$contact->contact_mails}}</td>
                                    <td>{{$contact->latitude}}</td>
                                    <td>{{$contact->longitude}}</td>
                                    <td>{{$contact->map_zoom}}</td>
                                    <td>
                                        <a href="{{route('user.contact.delete', $contact->id)}}" class="btn btn-danger">Delete</a>
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
