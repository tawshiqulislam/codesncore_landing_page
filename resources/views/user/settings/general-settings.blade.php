        ...
        <form id="ajaxForm" action="{{ route('user.general_settings.update_info') }}" method="post">
          ...
          <div class="card-body py-5">
            <div class="row">
              ...
              <div class="col-lg-6 offset-lg-3">
                <div class="form-group">
                  <label>{{ __('Users Login/Registration visibility') . '*' }}</label>
                  <div class="selectgroup w-100">
                    <label class="selectgroup-item">
                      <input type="radio" name="access_status" value="1" class="selectgroup-input"
                        {{ $data->access_status == 1 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Enable') }}</span>
                    </label>

                    <label class="selectgroup-item">
                      <input type="radio" name="access_status" value="0" class="selectgroup-input"
                        {{ $data->access_status == 0 ? 'checked' : '' }}>
                      <span class="selectgroup-button">{{ __('Disable') }}</span>
                    </label>
                  </div>
                  <p id="err_access_status" class="mb-0 text-danger em"></p>

                  <p class="text-warning mt-2 mb-0">
                    {{ __('If it is enable, the user will see login and registration button.') }}
                  </p>
                </div>
              </div>
            </div>
