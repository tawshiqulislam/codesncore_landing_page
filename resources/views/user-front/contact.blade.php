...
{{-- @dd($second) --}}
        @if(isset($second))
        <div><h1 class='text-center my-5'><u>Other Locations</u></h1></div>
            <div class="secondary-locations container">
                @foreach($second as $key => $sec)
                    <div class="row my-5">
                        @if($key % 2 == 0)
                        <div class="col-6 my-3" style="text-align: start">
                            <h5>{{ $sec->contact_form_title ?? '' }}</h5>
                            <h6>{{ $sec->contact_form_subtitle ?? '' }}</h6>
                            <p>{{ $sec->contact_addresses ?? '' }}</p>
                            <p>{{ $sec->contact_numbers ?? '' }}</p>
                            <p>{{ $sec->contact_mails ?? '' }}</p>
                        </div>
                        <div class="col-6 my-3" style="text-align: end">
                            @if (!empty($sec->latitude) && !empty($sec->longitude))
                                <div class="container-fluid container-300 mb-3">
                                    <div class="sec-contact-map">
                                        <iframe
                                            src="//www.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q={{ $sec->latitude ?? 36.7783 }},%20{{ $sec->longitude ?? 119.4179 }}+(My%20Business%20Name)&amp;t=&amp;z={{ $sec->map_zoom ?? 12 }}&amp;ie=UTF8&amp;iwloc=B&amp;output=embed"
                                            class="border-0" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div class="col-6 my-3" style="text-align: start">
                            @if (!empty($sec->latitude) && !empty($sec->longitude))
                                <div class="container-fluid container-300 mb-0">
                                    <div class="sec-contact-map">
                                        <iframe
                                            src="//www.google.com/maps?width=100%25&amp;height=600&amp;hl=en&amp;q={{ $sec->latitude ?? 36.7783 }},%20{{ $sec->longitude ?? 119.4179 }}+(My%20Business%20Name)&amp;t=&amp;z={{ $sec->map_zoom ?? 12 }}&amp;ie=UTF8&amp;iwloc=B&amp;output=embed"
                                            class="border-0" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>
